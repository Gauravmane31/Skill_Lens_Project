...
// Body: { name, email, password, accountType?: 'student'|'recruiter', company? }
const FREE_EMAIL_DOMAINS = new Set([
  "gmail.com", "yahoo.com", "hotmail.com", "outlook.com", "live.com",
  "icloud.com", "aol.com", "protonmail.com", "proton.me", "mail.com",
  "gmx.com", "yandex.com", "rediffmail.com", "zoho.com",
]);

app.post("/api/auth/signup", authLimiter, async (req, res) => {
  const { email, password } = req.body;
  const accountType = String(req.body?.accountType || "student").toLowerCase() === "recruiter" ? "recruiter" : "student";
  const company = String(req.body?.company || "").trim();
  const name = accountType === "recruiter" ? company : String(req.body?.name || "").trim();

  if (!name || !email || !password) {
    return res.status(400).json({ error: accountType === "recruiter" ? "Company name, work email and password are required." : "name, email and password are required." });
  }
  ...
  if (accountType === "recruiter") {
    if (!company) {
      return res.status(400).json({ error: "Company name is required for a recruiter account." });
    }
    const domain = String(email).split("@")[1]?.toLowerCase() || "";
    if (!domain || FREE_EMAIL_DOMAINS.has(domain)) {
      return res.status(400).json({ error: "Please sign up with your company (work) email address, not a personal email provider." });
    }
  }
  ...
  const { data, error } = await supabase.auth.signUp({
    ...
    password,
    options: {
      data: { full_name: name, role: accountType, company: accountType === "recruiter" ? company : null },
    },
  });
