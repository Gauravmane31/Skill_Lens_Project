...
function computeIntegrity(m) {
  ...
  const typingRatio = m.keystrokes / Math.max(1, m.keystrokes + m.pasteChars);
...
async function aiAnalysis(
  ...
  try {
    ...
    const { analyzeCodeWithAI } = await import("../services/aiApi.js");
...
async function jobSuggestions(
  ...
  try {
    const { getJobSuggestionsWithAI } = await import("../services/aiApi.js");
...
function fallbackJobSuggestions(avgCode, avgIntegrity) {
  ...
  if (avg >= 80)
    return [
      ...
      { role: "Full-Stack Engineer", prob: 71, color: C.green },
      { role: "Software Engineer II", prob: 65, color: C.amber },
    ];
  if (avg >= 65)
    return [
      { role: "Junior Developer", prob: 72, color: C.indigo },
      { role: "Frontend Developer", prob: 61, color: "#2F6FE0" },
      { role: "QA Engineer", prob: 55, color: C.green },
    ];
  return [
    { role: "Intern / Trainee", prob: 68, color: C.indigo },
    { role: "Technical Support", prob: 60, color: "#2F6FE0" },
  ];
}
...
async function skillGaps(codeScore, userProfile = {}, targetRole = "") {
  try {
    const { analyzeSkillGapsWithAI } = await import("../services/aiApi.js");
...
function fallbackSkillGaps(codeScore) {
  if (codeScore >= 80)
    return [
      "Practise system design patterns",
  ...
  if (codeScore >= 60)
    return [
      ...
      "Practise data structures daily",
      "Learn async/await",
    ];
  return [
    "Study Big-O complexity",
    "Practise LeetCode Easy problems",
    "Read Clean Code (Robert Martin)",
  ];
}

const skillGapsFallback = fallbackSkillGaps;
// Synchronous fallbacks for pages (Results, Dashboard) that render derived
// stats immediately rather than awaiting an AI call — aiAnalysis/jobSuggestions
// are async (they try an AI call first), so calling them without `await` returns
// a Promise, not data, which crashes any code that then does `.map()` on it.
const aiAnalysisFallback = fallbackAIAnalysis;
const jobSuggestionsFallback = fallbackJobSuggestions;
