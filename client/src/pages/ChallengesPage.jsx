...
import { C, CHALLENGES as STATIC_CHALLENGES } from "../constants/constants.js";
import useBreakpoint from "../hooks/useBreakpoint.js";
import { Card, Badge, ProgressBar, Pill, PageHero, inputSt } from "../components/common/Atoms.jsx";
    ...
    fetchProblems().then(data => {
      ...
      const merged = data.map(dbP => {
        ...
        return {
          ...
          company: staticMatch.tags?.[0] || "Company",
          logo: COMPANY_LOGOS[staticMatch.tags?.[0]] || null,
      ...
      <div className="sl-page-wrap" style={{padding:isMobile?"16px 14px":"20px 24px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 1fr":"repeat(5,1fr)",gap:12,marginBottom:18}}>
          {[
            ...
            <Card key={s.l} style={{padding:"12px 14px",textAlign:"center"}}>
              <div style={{fontWeight:700,fontSize:isMobile?18:22,color:s.a}}>{s.v}</div>
        ...
        {filtered.length===0?(
          <Card style={{textAlign:"center",padding:"40px 20px"}}>
            ...
            <h3 style={{fontWeight:600,marginBottom:6}}>No challenges match</h3>
          ...
          <div className="sl-grid-2">
            {filtered.map(ch=>{
              ...
              return(
                <div key={ch.id} className="sl-card-hover" style={{background:C.white,borderRadius:12,padding:isMobile?16:22,minWidth:0,cursor:"pointer",position:"relative",border:`1px solid ${C.border}`,borderLeft:`4px solid ${done?C.indigo:C.border}`}}
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:9,marginBottom:12}}>
                    <div style={{width:42,height:42,background:ch.company === "Palantir" ? "#000" : C.white,borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,border:`1px solid ${C.indigo}22`,overflow:"hidden"}}>
                      {ch.logo ? (
                        <img src={ch.logo} alt={`${ch.company} logo`} style={{width:ch.company === "Palantir" ? 36 : 28,height:ch.company === "Palantir" ? 24 : 28,objectFit:"contain"}} />
                      ) : (
                        <span style={{fontSize:10,fontWeight:700,color:C.indigo,textAlign:"center",lineHeight:1.1}}>{ch.company}</span>
                      )}
                    </div>
                    <div>
                  ...
                  <h3 style={{fontWeight:600,fontSize:isMobile?15:17,margin:"0 0 5px",color:C.text}}>{ch.title}</h3>
