...
import { Card, SectionHeader, PageHero, inputSt, Badge, ProgressBar } from "../components/common/Atoms.jsx";
import {
...
function RecruiterDashboardPage({ user }) {
              ...
              <Card>
                ...
                <div style={{ marginTop: 12, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                  <div style={{ background: C.bg, borderRadius: 10, border: `1px solid ${C.border}`, padding: 10 }}>
                    ...
                    <div style={{ fontSize: 18, color: C.indigo, fontWeight: 700 }}>{recruiterJobs.length}</div>
                  </div>
                  <div style={{ background: C.bg, borderRadius: 10, border: `1px solid ${C.border}`, padding: 10 }}>
                    <div style={{ fontSize: 11, color: C.muted }}>Matching Candidates</div>
                    <div style={{ fontSize: 18, color: C.indigo, fontWeight: 700 }}>{topCandidates.length}</div>
                  </div>
                </div>
              </Card>
            </div>
              ...
              <Card>
                      ...
                      return (
                        ...
                        >
                          <div style={{ display: "flex", justifyContent: "space-between", gap: 8, marginBottom: 4 }}>
                            <div style={{ fontWeight: 600, color: C.text }}>{test.company} - {test.roleTitle}</div>
                ...
                {selectedTestId ? (
                  <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${C.border}` }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: C.text, marginBottom: 8 }}>Manage Selected Test</div>
                    <input
                  ...
                  {testLeaderboard.map((row, index) => (
                    <div key={row.userId} style={{ border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 12px" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", gap: 8, flexWrap: "wrap", marginBottom: 6 }}>
                        <div style={{ fontWeight: 600, color: C.text }}>#{index + 1} {row.name}</div>
            ...
            <Card>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
                  {topCandidates.map((candidate) => (
                    <div key={candidate.userId} style={{ border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 12px" }}>
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", gap: 8, flexWrap: "wrap", marginBottom: 6 }}>
                        <div style={{ fontWeight: 600, color: C.text }}>{candidate.name}</div>
              ...
              {!!selectedTestId && (
                <div style={{ marginTop: 12, borderTop: `1px solid ${C.border}`, paddingTop: 10 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: C.text, marginBottom: 8 }}>Assigned Candidates For Selected Test</div>
                  {!assignments.length ? (
