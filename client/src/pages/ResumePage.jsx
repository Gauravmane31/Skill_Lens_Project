...
import { scoreColor, integrityLabel } from "../utils/scoring.js";
...
import {
  ...
  CircleScore,
  Avatar,
} from "../components/common/Atoms.jsx";
import { loadResumeProfile, saveResumeProfile } from "../services/api.js";
...
const roleWordRx =
  /(engineer|developer|intern|analyst|consultant|architect|manager|lead|scientist|specialist|full.?stack|frontend|backend|sde)/i;
const dateRangeRx =
  /(\b(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\b[\w\s,.-]*\d{2,4}|\b\d{4}\b)\s*(?:-|to|–|—)\s*(present|current|\b(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\b[\w\s,.-]*\d{2,4}|\b\d{4}\b)/i;
const shortDateRangeRx =
  /\b(19|20)\d{2}\b\s*(?:-|to|–|—)\s*\b(19|20)\d{2}|present|current\b/i;
...
const splitDates = (line = "") => {
  ...
  if (!m) {
    ...
    if (years.length === 1 && /present|current/i.test(line))
      return { from: years[0], to: "Present" };
    return { from: "", to: "" };
  }
...
const companyHintRx =
  /(inc\.?|llc|ltd\.?|pvt\.?\s*ltd\.?|technologies|tech|solutions|systems|labs|corp\.?|company|services|consulting|software|private limited)/i;
const locationHintRx =
  /(remote|on-?site|hybrid|india|usa|uk|canada|singapore|bangalore|bengaluru|hyderabad|mumbai|pune|delhi|gurugram|chennai)/i;
...
const looksLikeContactNoise = (line = "") =>
...
const placeholderValueRx =
  /^(n\/?a|na|none|null|nil|not\s+available|not\s+provided|unknown|tbd|--|-)$/i;
...
const parseExperienceCompositeLine = (line = "") => {
  ...
  const datePart =
    parts.find((p) => dateRangeRx.test(p) || shortDateRangeRx.test(p)) || "";
  const { from, to } = splitDates(datePart);
  const titlePart = parts.find((p) => roleWordRx.test(p)) || "";
  const companyPart =
    parts.find((p) => p !== titlePart && looksLikeCompany(p)) || "";
...
const looksLikeInstitution = (line = "") =>
  instituteRx.test(line) || looksLikeCompany(line);
...
const gpaRx =
  /(cgpa|gpa)\s*[:\-]?\s*([0-9]+(?:\.[0-9]+)?(?:\s*\/\s*[0-9]+(?:\.[0-9]+)?)?)/i;
...
const parseExperienceEntries = (lines = []) => {
  ...
  for (let i = 0; i < clean.length; i += 1) {
    ...
    const title =
      [prev1, prev2, next1].find(
        (v) =>
          roleWordRx.test(v) && v.length <= 90 && !looksLikeContactNoise(v),
      ) || "";
    const company =
      [prev1, prev2, next1, next2].find(
        (v) => v !== title && looksLikeCompany(v),
      ) || "";
    const location =
      [prev1, prev2, next1, next2].find((v) => looksLikeLocation(v)) || "";
  ...
  if (!entries.length) {
    for (let i = 0; i < clean.length; i += 1) {
      ...
      const n2 = clean[i + 2] || "";
      const dateSource =
        [line, next, n2].find((v) => dateRangeRx.test(v)) || "";
      ...
      entries.push({
        ...
        location: normalizeTextValue(
          looksLikeLocation(next) ? next : looksLikeLocation(n2) ? n2 : "",
        ),
        from: normalizeTextValue(from),
        to: normalizeTextValue(to),
        description: [n2, clean[i + 3] || ""]
  ...
  return dedupeByKey(
    entries,
    (e) =>
      `${(e.title || "").toLowerCase()}|${(e.company || "").toLowerCase()}|${e.from}|${e.to}`,
  )
...
const degreeRx =
  /(b\.tech|btech|be\b|b\.e\.|m\.tech|mtech|bca|mca|bsc|msc|bachelor|master|mba|phd)/i;
...
const parseEducationEntries = (lines = []) => {
  ...
  for (let i = 0; i < clean.length; i += 1) {
    ...
    if (
      ...
      !dateRangeRx.test(line)
    )
      continue;

    const degreeLine =
      [line, next, prev].find(
        (v) => looksLikeDegree(v) && !looksLikeContactNoise(v),
      ) || "";
    const institution =
      [line, next, prev].find(
        (v) =>
          looksLikeInstitution(v) &&
          !looksLikeDegree(v) &&
          !looksLikeContactNoise(v),
      ) || "";
    const dateLine =
      [line, next, prev, clean[i + 2] || ""].find(
        (v) => dateRangeRx.test(v) || /\b(19|20)\d{2}\b/.test(v),
      ) || "";
    const { from, to } = splitDates(dateLine);
    const gpaLine =
      [line, next, prev, clean[i + 2] || ""].find((v) => gpaRx.test(v)) || "";
  ...
  return dedupeByKey(
    entries,
    (e) =>
      `${(e.degree || "").toLowerCase()}|${(e.institution || "").toLowerCase()}|${e.from}|${e.to}`,
  )
...
const mergeParsedProfile = (prev, parsed) => {
  ...
  if (parsed.fullName && isLikelyName(parsed.fullName))
    next.fullName = parsed.fullName;
  if (parsed.email && /@/.test(parsed.email)) next.email = parsed.email;
  if (parsed.phone && parsed.phone.replace(/\D/g, "").length >= 8)
    next.phone = parsed.phone;
  if (parsed.location && parsed.location.length >= 3)
    next.location = parsed.location;

  if (parsed.headline && roleWordRx.test(parsed.headline))
    next.headline = parsed.headline;
  if (parsed.summary && parsed.summary.length >= 20)
    next.summary = parsed.summary;
  if (parsed.jobTitle && roleWordRx.test(parsed.jobTitle))
    next.jobTitle = parsed.jobTitle;
  ...
  if (parsed.linkedin && /linkedin\.com/i.test(parsed.linkedin))
    next.linkedin = parsed.linkedin;
  if (parsed.github && /github\.com/i.test(parsed.github))
    next.github = parsed.github;
  if (parsed.portfolio && !/linkedin\.com|github\.com/i.test(parsed.portfolio))
...
const findSectionIndex = (lines, patterns) =>
  lines.findIndex((line) => patterns.some((rx) => rx.test(line)));
...
const collectSectionLines = (lines, startIndex) => {
  ...
  for (let i = startIndex + 1; i < lines.length; i += 1) {
    const line = lines[i];
    const isNextHeader = Object.values(sectionPatterns).some((patterns) =>
      patterns.some((rx) => rx.test(line)),
    );
...
const extractResumeData = (rawText, fallbackName = "", fallbackEmail = "") => {
  ...
  const allUrls = Array.from(text.matchAll(/https?:\/\/[^\s)\]]+/gi)).map(
    (m) => m[0],
  );
  ...
  const locationLine =
    topSlice.find(
      (line) =>
        /(,\s*[A-Za-z]{2,}|india|usa|united states|remote|bengaluru|bangalore|hyderabad|pune|mumbai|delhi|gurugram|chennai)/i.test(
          line,
        ) && !line.includes("@"),
    ) || "";
  ...
  const roleLine =
    topSlice.find(
      (line) => roleWordRx.test(line) && !looksLikeContactNoise(line),
    ) || "";
  ...
  const skillsSection = collectSectionLines(
    lines,
    findSectionIndex(lines, sectionPatterns.skills),
  );
  const languageSection = collectSectionLines(
    lines,
    findSectionIndex(lines, sectionPatterns.languages),
  );
  const certSection = collectSectionLines(
    lines,
    findSectionIndex(lines, sectionPatterns.certifications),
  );
  ...
  const skills = uniq(
    skillsSection.length
      ? [...splitTokens(skillsSection), ...skillsFromKeywords]
      : skillsFromKeywords,
  ).slice(0, 30);
  const languages = uniq(
    languageSection.length ? splitTokens(languageSection) : [],
  ).slice(0, 12);
  const certifications = uniq(
    certSection.length ? splitTokens(certSection) : [],
  ).slice(0, 16);

  const expLines = collectSectionLines(
    lines,
    findSectionIndex(lines, sectionPatterns.experience),
  );
  const experience = parseExperienceEntries(expLines);

  const eduLines = collectSectionLines(
    lines,
    findSectionIndex(lines, sectionPatterns.education),
  );
  const education = parseEducationEntries(eduLines);

  const topText = topSlice.join(" ");
  const topUrls = Array.from(topText.matchAll(/https?:\/\/[^\s)\]]+/gi)).map(
    (m) => m[0],
  );
  const portfolio =
    topUrls.find((url) => !/linkedin\.com|github\.com/i.test(url)) || "";
  ...
  return {
    ...
    jobType: /internship|intern\b/i.test(lower)
      ? "Internship"
      : /part[- ]?time/i.test(lower)
        ? "Part-time"
        : /full[- ]?time/i.test(lower)
          ? "Full-time"
          : "",
    remotePreference: /hybrid/i.test(lower)
      ? "Hybrid"
      : /remote/i.test(lower)
        ? "Remote"
        : /on[- ]?site|onsite/i.test(lower)
          ? "On-site"
          : "",
...
const extractTextFromPdf = async (file) => {
  const [{ GlobalWorkerOptions, getDocument }, pdfWorkerModule] =
    await Promise.all([
      import("pdfjs-dist"),
...
function ResumePage({ user, results }) {
  ...
  const handleFile = async (f) => {
    ...
    try {
      ...
      setResumeText(text);
      const parsed = extractResumeData(
        text,
        user?.name || "",
        user?.email || "",
      );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const missingTable =
        /resume_profiles|public\.resume_profiles|schema cache|table not found/i.test(
          message,
        );
      if (missingTable) {
        localStorage.setItem(localResumeKey, JSON.stringify(profile));
        setSaveMessage(
          "Backend resume table is missing. Profile saved locally in this browser. Run supabase/schema.sql in Supabase SQL Editor to enable cloud save.",
        );
      } else {
    ...
    <div style={{ overflowY: "auto", flex: 1, background: C.bg }}>
      {!parseSuccess && !file ? (
        <div
          style={{
            background:
              "linear-gradient(135deg,#4438D6 0%,#7A70E0 50%,#1B1E2C 100%)",
        ...
        >
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              ...
              >
                Hey {user?.name?.split(" ")[0]}! Build your profile.
              </h1>
              <p
                style={{
              ...
              >
                Upload your resume — AI reads it and fills every field in
                seconds.
              </p>
            </div>
        ...
        <PageHero
          ...
          sub="Upload your resume — AI fills your profile instantly."
          extras={
            <div
              style={{
                display: "flex",
            ...
            >
              ...
              >
                <div style={{ fontSize: 10, color: "#6B6F7B", marginBottom: 4 }}>
              ...
              {latest && (
                ...
                >
                  <div style={{ fontSize: 10, color: "#6B6F7B", marginBottom: 1 }}>
                    LAST SCORE
                  </div>
                  <div
                    style={{
                      fontWeight: 700,
                      fontSize: 16,
                      color: scoreColor(latest.codeScore),
                    }}
                  >
                    {latest.codeScore}
                  </div>
      ...
      <div
        className="sl-page-wrap"
        style={{ padding: isMobile ? "16px 14px" : "20px 24px" }}
      >
        {/* Upload zone */}
        <Card
          style={{
            marginBottom: 16,
            border: !parseSuccess && !file ? `2px solid ${C.indigo}` : "none",
            boxShadow:
              !parseSuccess && !file
                ? `0 0 0 4px ${C.indigoLight}`
                : "none",
          }}
        >
          <div
          ...
          >
            <div>
              <h2 style={{ fontWeight: 700, fontSize: 15, margin: "0 0 2px" }}>
                📄 Upload Resume
              </h2>
              <p style={{ color: C.muted, fontSize: 12, margin: 0 }}>
                {parseSuccess
                  ? "Parsed — edit your details below"
                  : "AI reads and fills all fields automatically"}
              </p>
            </div>
            {parseSuccess && (
              <button
          ...
          <div
            ...
            style={{
              border: `2px dashed ${dragOver ? C.indigo : parseSuccess ? C.green : C.border}`,
              borderRadius: 12,
              padding: "28px 20px",
              textAlign: "center",
              cursor: parseSuccess ? "default" : "pointer",
              background: dragOver
                ? C.indigoLight
                : parseSuccess
                  ? C.pastelGreen
        ...
        <div className="sl-two-col">
          ...
          <div>
              ...
              {sectionTabs.map((t) => (
                <button
                  ...
                  style={{
                    padding: "7px 12px",
                    borderRadius: 9,
                    border: `1.5px solid ${activeTab === t.id ? C.indigo : C.border}`,
                    background: activeTab === t.id ? C.indigo : C.white,
            ...
            {activeTab === "personal" && (
              <Card>
                <h3
                  style={{ fontWeight: 600, fontSize: 15, margin: "0 0 14px" }}
                >
                  👤 Personal Info
                </h3>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="Full-Stack Developer with 3 years experience"
                    style={inputSt}
                  />
                </div>
                <div>
            ...
            {activeTab === "skills" && (
              <Card>
                <h3
                  style={{ fontWeight: 600, fontSize: 15, margin: "0 0 16px" }}
                >
                  ⚡ Skills & Languages
                </h3>
              ...
              <Card>
                ...
                <div className="sl-grid-2">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => up({ jobTitle: e.target.value })}
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => up({ jobType: e.target.value })}
                      style={{ ...inputSt }}
                    >
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => up({ remotePreference: e.target.value })}
                      style={{ ...inputSt }}
                    >
                      <option value="">Select…</option>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => up({ noticePeriod: e.target.value })}
                    placeholder="30 days"
                    style={inputSt}
                  />
                </div>
            ...
            >
              <span
                style={{
                  fontSize: 12,
                  color: saveMessage.includes("success") ? C.green : C.muted,
                }}
              >
                {saveMessage}
              </span>
              <button
                ...
                style={{
                  padding: "11px 28px",
                  background: saving ? C.border : C.indigo,
                  color: "#fff",
                  border: "none",
                  borderRadius: 10,
                  fontWeight: 700,
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <Card style={{ background: C.dark }}>
              <div style={{ textAlign: "center", marginBottom: 14 }}>
                ...
                <div style={{ fontSize: 12, color: "#8A8E99", marginTop: 3 }}>
                  {profile.headline || "Add a headline…"}
                </div>
              </div>
              <div style={{ marginBottom: 10 }}>
                <div style={{ fontSize: 10, color: "#4A4E5A", marginBottom: 5 }}>
                  PROFILE COMPLETENESS
                </div>
                <ProgressBar
                  value={completeness}
              ...
              {[
                ...
                >
                  <span style={{ fontSize: 12, color: "#6B6F7B" }}>{l}</span>
                  <span
                    style={{
                      fontSize: 12,
                      fontWeight: 600,
                      color: v > 0 ? C.indigoMid : "#3A3D46",
                    }}
                  >
                    {v} {v === 1 ? "item" : "items"}
                  </span>
            ...
            {latest && (
              <Card style={{ background: C.indigoLight }}>
                ...
                >
                  <CircleScore
                    value={latest.codeScore}
                    size={60}
                    label="Code"
                  />
                  <CircleScore
                    value={latest.integrityScore}
                    size={60}
                    label="Integrity"
                    color={integrityLabel(latest.integrityScore).color}
                  />
                </div>
              </Card>
            )}
            ...
            <Card>
              <h4 style={{ fontWeight: 600, fontSize: 13, margin: "0 0 10px" }}>
                💡 Profile Tips
              </h4>
              {[
                {
                  tip: "Add a headline that shows your specialisation",
                  done: !!profile.headline,
                },
                {
                  tip: "Upload your resume to auto-fill all fields",
                  done: parseSuccess,
                },
                {
                  tip: "Add 5+ technical skills for better job matching",
                  done: profile.skills.length >= 5,
                },
                {
                  tip: "Include at least one work experience",
                  done: profile.experience.length > 0,
                },
                {
                  tip: "Set your job preferences for targeted matches",
                ...
                >
                  <div
                    style={{
                      ...
                      height: 16,
                      borderRadius: "50%",
                      background: t.done ? C.green : C.border,
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      fontSize: 9,
                  ...
                  <span
                    style={{
                      fontSize: 11,
                      color: t.done ? C.muted : C.textMid,
