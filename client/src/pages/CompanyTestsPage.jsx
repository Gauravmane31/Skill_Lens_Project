...
import { PageHero, Card, SectionHeader, Badge, ProgressBar } from "../components/common/Atoms.jsx";
import { fetchMyAssignedCompanyTests, fetchMyCompanyTestProgress } from "../services/api.js";
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {tests.map((test) => {
                  ...
                  return (
                    ...
                    >
                      <div style={{ fontWeight: 600, color: C.text, marginBottom: 4 }}>{test.company} - {test.roleTitle}</div>
                ...
                <>
                  <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(4,1fr)", gap: 10, marginBottom: 12 }}>
                    <div style={{ border: `1px solid ${C.border}`, borderRadius: 10, padding: 10 }}>
                      ...
                      <div style={{ fontWeight: 700, color: C.indigo }}>{selectedTestDetails.progress.completionRate}%</div>
                    </div>
                    <div style={{ border: `1px solid ${C.border}`, borderRadius: 10, padding: 10 }}>
                      <div style={{ fontSize: 10, color: C.muted }}>Final Score</div>
                      <div style={{ fontWeight: 700, color: C.green }}>{selectedTestDetails.progress.finalScore}%</div>
                    </div>
                    <div style={{ border: `1px solid ${C.border}`, borderRadius: 10, padding: 10 }}>
                      <div style={{ fontSize: 10, color: C.muted }}>Code Avg</div>
                      <div style={{ fontWeight: 700, color: C.text }}>{selectedTestDetails.progress.avgCode}%</div>
                    </div>
                    <div style={{ border: `1px solid ${C.border}`, borderRadius: 10, padding: 10 }}>
                      <div style={{ fontSize: 10, color: C.muted }}>Integrity Avg</div>
                      <div style={{ fontWeight: 700, color: C.text }}>{selectedTestDetails.progress.avgIntegrity}%</div>
                    </div>
                  </div>
