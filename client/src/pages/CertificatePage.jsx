...
import { integrityLabel, scoreColor } from "../utils/scoring.js";
import useBreakpoint from "../hooks/useBreakpoint.js";
import { PageHero, Pill, Badge, CircleScore, Card, Avatar } from "../components/common/Atoms.jsx";
      ...
      <div className="sl-page-wrap" style={{padding:isMobile?"16px 14px":"20px 24px"}}>
        {!latest?(
          <div style={{textAlign:"center",padding:"60px 20px"}}>
            ...
            <h3 style={{fontWeight:600,color:C.text,marginBottom:8}}>No certificates yet</h3>
          ...
          <>
            ...
            <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
                ...
                <div className="sl-certificate-print" style={{background:C.white,borderRadius:14,padding:isMobile?24:44,border:`2px solid ${C.indigoLight}`,position:"relative",overflow:"hidden"}}>
                  ...
                  <div style={{position:"relative"}}>
                    ...
                    <div style={{textAlign:"center",marginBottom:24}}>
                      ...
                      <div style={{fontSize:isMobile?22:32,fontWeight:700,marginBottom:6}}>{user?.name}</div>
                      ...
                      <div style={{display:"inline-flex",alignItems:"center",gap:10,background:latest.challenge.pastel,borderRadius:14,padding:"10px 20px",marginBottom:8}}>
                        ...
                        <span style={{fontSize:isMobile?16:20,fontWeight:700}}>{latest.challenge.title}</span>
                      </div>
              ...
              <div style={{flex:"0 0 260px",display:"flex",flexDirection:"column",gap:12}}>
                <Card>
                  <h3 style={{fontWeight:600,fontSize:14,margin:"0 0 12px"}}>📤 Share Certificate</h3>
                  {[
                    {icon:"💼",label:"LinkedIn",color:"#0A66C2",bg:"#E8F0FE"},
                    {icon:"𝕏",label:"X / Twitter",color:"#12141C",bg:"#F1F1EE"},
                ...
                <Card style={{background:C.dark}}>
                  <h3 style={{fontWeight:600,fontSize:14,margin:"0 0 12px",color:"#fff"}}>📊 Session Stats</h3>
                  {[
                    ["Code Score",`${latest.codeScore}/100`,scoreColor(latest.codeScore)],
                    ["Integrity",`${latest.integrityScore}/100`,il.color],
                    ["Time Taken",`${Math.floor(latest.metrics.typingDuration/60)}m ${latest.metrics.typingDuration%60}s`,"#9C9FA8"],
                    ["Keystrokes",latest.metrics.keystrokes,"#9C9FA8"],
                    ...
                    <div key={k} style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderBottom:`1px solid rgba(255,255,255,.06)`}}>
                      <span style={{fontSize:12,color:"#6B6F7B"}}>{k}</span>
                ...
                <Card style={{background:C.indigoLight}}>
                  <h3 style={{fontWeight:600,fontSize:13,margin:"0 0 8px",color:C.indigo}}>💡 Pro Tip</h3>
