...
import { computeIntegrity, computeCodeScore } from "../utils/scoring.js";
import useBreakpoint from "../hooks/useBreakpoint.js";
import { Badge } from "../components/common/Atoms.jsx";
import { submitCode } from "../services/api.js";
import { BOILERPLATE } from "../constants/boilerplate.js";
import { C } from "../constants/constants.js";
import { getBackendBaseUrl } from "../utils/runtimeConfig.js";

const COMPILER_API = getBackendBaseUrl();
const Editor = React.lazy(() => import("@monaco-editor/react"));
const LazyProctoring = React.lazy(() => import("../components/Proctoring.jsx"));

// ── Session Page ──────────────────────────────────────────────────────────────
function SessionPage({ challenge, onSubmit, setPage }) {
  const [lang, setLang] = useState("javascript");
  const getInitialCode = (requestedLang) =>
    BOILERPLATE[requestedLang] || challenge?.starterCode?.[requestedLang] || "";
  const [code, setCode] = useState(getInitialCode(lang));
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden) {
        ...
        toast.error(
          "⚠️ Warning: Tab switching detected! This lowers your integrity score.",
          { duration: 4000 },
        );
      }
    };
  ...
  const handleProctoringViolation = (type, details) => {
    ...
    if (type === "DISQUALIFIED") {
      toast.error(`🚫 Test Disqualified: ${details.reason}`, {
        duration: 6000,
      });
      if (document.fullscreenElement && document.exitFullscreen) {
        document.exitFullscreen().catch((e) => console.log(e));
  ...
  if (!challenge)
    return (
      ...
      >
        ...
        <h3 style={{ fontWeight: 600, color: C.text }}>
          No challenge selected
        </h3>
        <p style={{ color: C.muted, textAlign: "center" }}>
          Go to Challenges to pick one and start solving!
        </p>
        <button
    ...
    if (testCases && testCases.length > 0) {
      try {
        ...
        for (let i = 0; i < testCases.length; i++) {
          const testLine = outputLines.find((line) =>
            line.includes(`Test ${i + 1}:`),
          );
          if (testLine) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.error(err.message || "Error submitting to backend");
      setRunning(false);
      return;
    }
    ...
    const finalScore = backendResult
      ? backendResult.score
      : computeCodeScore(currentCode);
    const finalFeedback = backendResult || {
      strengths: [],
      weaknesses: [],
      improvementTips: [],
      conceptGaps: [],
  ...
  return (
    <div className="sl-session">
          ...
          {activeTab === "problem" && (
            <>
              ...
              >
                ...
                <span
                  style={{
                    ...
                    fontSize: 13,
                    background:
                      elapsed > challenge.timeLimit * 60
                        ? "#F7DAD6"
                        : C.indigoLight,
                    color:
                      elapsed > challenge.timeLimit * 60 ? C.red : C.indigo,
                    padding: "4px 9px",
                    borderRadius: 8,
              ...
              >
                ...
                <div>
                  <h2
                    style={{ fontWeight: 700, fontSize: 15, margin: "0 0 3px" }}
                  >
                    {challenge.title}
                  </h2>
              ...
              >
                ...
                {[
                  ...
                  ["Paste Events", metricsRef.current.pasteEvents],
                  ["Est. Code Score", `${codeScore}/100`],
                  ["Est. Integrity", `${integrity}/100`],
                ].map(([k, v]) => (
                  <div
                    key={k}
                    style={{
                      ...
                      fontSize: 12,
                      padding: "4px 0",
                      borderBottom: `1px solid ${C.indigo}18`,
                    }}
                  >
          ...
          {activeTab === "hints" && (
            <div>
              <p style={{ fontSize: 12, color: C.muted, marginBottom: 14 }}>
                💡 Stuck? Here are some hints — use them sparingly for max
                integrity score.
              </p>
              {challenge.hints.map((h, i) => (
                ...
                >
                  ...
                  <div
                    style={{ fontSize: 13, color: C.textMid, lineHeight: 1.5 }}
                  >
                    {h}
                  </div>
                </div>
              ...
              >
                <p style={{ fontSize: 12, color: C.muted, margin: 0 }}>
                  ⚠️ Using hints may reduce your integrity score. Think it
                  through first!
                </p>
              </div>
            </div>
          ...
          {activeTab === "tests" && (
            <div>
              <p style={{ fontSize: 12, color: C.muted, marginBottom: 12 }}>
                🧪 Test cases — run your code to see results.
              </p>
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      padding: "11px 13px",
                      marginBottom: 8,
                      border: `1px solid ${res ? (res.passed ? C.green : C.amber) : C.border}`,
                    }}
                  >
                    <div
                      style={{
                    ...
                    >
                      ...
                      {res && (
                        <Badge
                          label={res.passed ? "✅ PASS" : "❌ FAIL"}
                          color={res.passed ? C.green : C.red}
                        />
                      )}
                    </div>
                    <div
      ...
      >
        ...
        >
          <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
            ...
            {["javascript", "python", "java", "cpp", "c", "go", "rust"].map(
              (l) => (
                <button
                  key={l}
                  onClick={() => setLang(l)}
                  style={{
                    ...
                    borderRadius: 7,
                    border: "none",
                    background: lang === l ? C.indigo : "#1B1E2C",
                    color: lang === l ? "#fff" : "#6B6F7B",
                    fontWeight: 600,
                    fontSize: 11,
                    cursor: "pointer",
                  }}
          ...
          <div style={{ display: "flex", gap: 6 }}>
            <button
              ...
              style={{
                padding: "6px 13px",
                background: running ? "#1B1E2C" : C.green,
                color: "#fff",
                border: "none",
                borderRadius: 8,
                fontWeight: 700,
            ...
            <button
              ...
              style={{
                padding: "6px 14px",
                background: running ? "#777" : C.indigo,
                color: "#fff",
                border: "none",
                borderRadius: 8,
                fontWeight: 700,
            ...
            <Editor
              ...
              onMount={(editor) => {
                ...
                editor.onDidPaste((e) => {
                  ...
                  metricsRef.current.largestPaste = Math.max(
                    metricsRef.current.largestPaste,
                    pasted.split("\n").length,
                  );
                  tick((t) => t + 1);
                });
        ...
        >
          ...
          >
            <button
              ...
              style={{
                ...
                border: "none",
                borderBottom: !showStdin
                  ? "2px solid #3A2FC9"
                  : "2px solid transparent",
                color: !showStdin ? "#E4E4E1" : "#4A4E5A",
                fontSize: 11,
                fontWeight: 700,
                cursor: "pointer",
                textTransform: "uppercase",
          ...
          {!showStdin && (
            <div style={{ flex: 1, overflowY: "auto", padding: "10px 16px" }}>
              {running ? (
                <div style={{ color: "#6B6F7B", fontSize: 12 }}>
                  Running
                  <span
                    style={{
                ...
                <div
                  style={{ color: "#23252C", fontSize: 12, fontStyle: "italic" }}
                >
      ...
      <React.Suspense fallback={null}>
        <LazyProctoring onViolation={handleProctoringViolation} />
      </React.Suspense>
    </div>
  );
}
