...
import { fetchProgressInsights } from "../services/api.js";
import { supabase } from "../services/supabase.js";
import useBreakpoint from "../hooks/useBreakpoint.js";
import { PageHero, Card, SectionHeader, CircleScore, ProgressBar, Badge } from "../components/common/Atoms.jsx";
...
function TrendChart({ data, isMobile }) {
  ...
  return (
    <div ref={chartRef} style={{ position: "relative", overflowX: isMobile ? "auto" : "hidden" }}>
        ...
        {pts.map((p, i) => (
          <g
            ...
            onMouseEnter={(event) => {
              ...
              if (chart) setHoverPosition({ x: point.left + point.width / 2 - chart.left, y: point.top + point.height / 2 - chart.top });
            }}
            onMouseLeave={() => { setHovered(null); setHoverPosition(null); }}
      ...
      {/* Tooltip */}
      {hovered !== null && pts[hovered] && hoverPosition && (
        <div style={{
          position: "absolute",
          ...
          <div style={{ display: "flex", gap: 12 }}>
            <span><span style={{ color: C.indigo, fontWeight: 600 }}>●</span> Code: {pts[hovered].code_score}</span>
            <span><span style={{ color: C.green, fontWeight: 600 }}>●</span> Integrity: {pts[hovered].integrity_score}</span>
          </div>
...
// ── Progress Page ────────────────────────────────────────────────────────────
function ProgressPage({ user, setPage }) {
  ...
  const daysSinceFirst = new Set(
    chronological.map((submission) => new Date(submission.created_at).toDateString()),
  ).size;
  ...
  if (!submissions.length) {
    return (
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 14, background: C.bg, padding: 24 }}>
        ...
        <h3 style={{ fontWeight: 600, color: C.text }}>No progress data yet</h3>
    ...
    <div style={{ overflowY: "auto", flex: 1, background: C.bg }}>
      ...
      <PageHero
        ...
        extras={
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {[
              { label: "TOTAL SESSIONS", val: submissions.length, color: "#D2D2CE" },
              { label: "DAYS ACTIVE", val: daysSinceFirst, color: "#D2D2CE" },
      ...
      <div className="sl-page-wrap" style={{ padding: isMobile ? "16px 14px" : "20px 24px" }}>
        ...
        {submissions.length > 0 && (
          ...
          >
            📊 View your latest session report →
          </button>
        )}
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(4, 1fr)", gap: 12, marginBottom: 18 }}>
          {[
            ...
            <Card key={i} style={{ padding: "14px 16px", textAlign: "center" }}>
              ...
              {c.delta !== undefined ? (
                <>
                  <div style={{ fontWeight: 700, fontSize: 22, color: c.delta >= 0 ? C.green : C.red }}>
                    {c.after}
                  </div>
                ...
                <>
                  <div style={{ fontWeight: 700, fontSize: 22, color: C.indigo }}>{c.val}</div>
            ...
            <>
              ...
              {improving.length > 0 && (
                <div style={{ marginBottom: 14 }}>
                  <div style={{ fontSize: 10, fontWeight: 600, color: C.green, textTransform: "uppercase", letterSpacing: .5, marginBottom: 8, display: "flex", alignItems: "center", gap: 5 }}>
                    ...
                    {improving.map(t => (
                      <div key={t.tag} style={{
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                          <span style={{ fontWeight: 600, fontSize: 13, color: C.text }}>{t.tag}</span>
                          <span style={{
                        ...
                        <div style={{ display: "flex", gap: 4, alignItems: "center", marginBottom: 4 }}>
                          ...
                          <span style={{ fontSize: 11, fontWeight: 600, color: C.green, width: 28, textAlign: "right" }}>{t.secondAvg}</span>
                        </div>
              ...
              {stable.length > 0 && (
                <div style={{ marginBottom: 14 }}>
                  <div style={{ fontSize: 10, fontWeight: 600, color: C.amber, textTransform: "uppercase", letterSpacing: .5, marginBottom: 8, display: "flex", alignItems: "center", gap: 5 }}>
              ...
              {declining.length > 0 && (
                <div>
                  <div style={{ fontSize: 10, fontWeight: 600, color: C.red, textTransform: "uppercase", letterSpacing: .5, marginBottom: 8, display: "flex", alignItems: "center", gap: 5 }}>
                    ...
                    {declining.map(t => (
                      <div key={t.tag} style={{
                        ...
                        <div style={{ display: "flex", gap: 4, alignItems: "center", marginBottom: 4 }}>
                          ...
                          <span style={{ fontSize: 11, fontWeight: 600, color: C.red, width: 28, textAlign: "right" }}>{t.secondAvg}</span>
                        </div>
                        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 9, color: C.muted }}>
                          <span>{t.count} sessions</span>
                          <span>was {t.firstAvg} → now {t.secondAvg}</span>
                ...
                {[
                  ...
                  return (
                    <div key={i} style={{
                      ...
                      <div style={{ flex: 1, textAlign: "center", fontWeight: 600, fontSize: 16, color: C.muted }}>{m.b}</div>
                      ...
                      <div style={{ flex: 1, textAlign: "center", fontWeight: 600, fontSize: 16, color: improved !== false ? C.indigo : C.red }}>{m.a}</div>
                    </div>
                  );
                })}
              </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(3, 1fr)", gap: 10 }}>
            {languages.map(l => (
              <div key={l.lang} style={{
                ...
                <div style={{ fontWeight: 600, fontSize: 14, color: C.text, marginBottom: 8 }}>{l.label}</div>
                <CircleScore value={l.avg} size={60} label="Avg Score" />
                <div style={{ display: "flex", justifyContent: "space-around", marginTop: 10 }}>
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 14, color: C.indigo }}>{l.count}</div>
                    <div style={{ fontSize: 9, color: C.muted }}>Sessions</div>
                  </div>
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 14, color: C.green }}>{l.best}</div>
            ...
            {timelineData.map((sub, i) => {
              ...
              return (
                <div key={sub.created_at + i} style={{
                  ...
                  {/* Timeline dot */}
                  <CompanyLogo company={ch?.tags?.[0]} fallback={ch?.icon || "⌨️"} size={24} />
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
                    ...
                    <div style={{ textAlign: "center" }}>
                      <div style={{ fontWeight: 600, fontSize: 16, color: scoreColor(Number(sub.code_score)) }}>{sub.code_score}</div>
        ...
        <div style={{
          ...
          <div>
            <h3 style={{ fontWeight: 600, fontSize: 15, color: "#fff", margin: "0 0 3px" }}>Keep improving! 🔥</h3>
            <p style={{ color: "#6B6F7B", fontSize: 13, margin: 0 }}>Practice more challenges to see your scores climb.</p>
          </div>
