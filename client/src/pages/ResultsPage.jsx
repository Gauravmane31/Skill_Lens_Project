...
import { scoreColor, integrityLabel, aiAnalysisFallback, jobSuggestionsFallback, skillGapsFallback } from "../utils/scoring.js";
import useBreakpoint from "../hooks/useBreakpoint.js";
import { PageHero, Card, SectionHeader, CircleScore, ProgressBar, Pill, Badge } from "../components/common/Atoms.jsx";
...
function ResultsPage({results,setPage}){
  ...
  if(!results.length) return(
    <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:14,background:C.bg,padding:24}}>
      ...
      <h3 style={{fontWeight:600,color:C.text}}>No results yet</h3>
  ...
  const m = {
    pasteEvents: r.metrics?.pasteEvents || 0,
    keystrokes: r.metrics?.keystrokes || 0,
    typingDuration: r.metrics?.typingDuration || 0,
    tabSwitches: r.metrics?.tabSwitches || 0,
    largestPaste: r.metrics?.largestPaste || 0,
    pasteChars: r.metrics?.pasteChars || 0,
  };

  const backend = r.backendResult || {};
  const strengths = (backend.strengths && backend.strengths.length > 0) ? backend.strengths : aiAnalysisFallback(r.codeScore).strengths;
  const improvements = (backend.improvementTips && backend.improvementTips.length > 0) ? backend.improvementTips :
                       (backend.weaknesses && backend.weaknesses.length > 0) ? backend.weaknesses : aiAnalysisFallback(r.codeScore).improvements;
  const gaps = (backend.conceptGaps && backend.conceptGaps.length > 0) ? backend.conceptGaps : skillGapsFallback(r.codeScore);

  const roles = jobSuggestionsFallback(r.codeScore, r.integrityScore);
    ...
    <div style={{overflowY:"auto",flex:1,background:C.bg}}>
        ...
        extras={
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            {[
              ...
              {label:"CHALLENGE",val:r.challenge.title,color:"#D2D2CE",small:true},
              {label:"LANGUAGE",val:r.lang,color:"#9C9FA8",small:true},
            ].map(s=>(
              <div key={s.label} style={{background:"rgba(255,255,255,.07)",borderRadius:10,padding:"8px 13px",backdropFilter:"blur(4px)"}}>
                <div style={{fontSize:9,color:"#6B6F7B",marginBottom:2,textTransform:"uppercase",letterSpacing:.5}}>{s.label}</div>
                <div style={{fontWeight:700,fontSize:s.small?12:20,color:s.color}}>{s.val}</div>
              </div>
            ))}
          </div>
        }
      ...
      <div className="sl-page-wrap" style={{padding:isMobile?"16px 14px":"20px 24px"}}>
        <button
          ...
          style={{ display: "flex", alignItems: "center", gap: 6, background: C.white, border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 14px", marginBottom: 16, cursor: "pointer", fontSize: 13, fontWeight: 600, color: C.indigo, width: "fit-content" }}
        >
        ...
        <div className="sl-grid-2" style={{marginBottom:14}}>
          ...
          <Card>
            ...
            <div style={{background:il.color+"12",borderRadius:10,padding:"10px 13px",borderLeft:`3px solid ${il.color}`,marginBottom:10}}>
              ...
              <div style={{fontSize:11,color:C.muted,marginTop:2}}>{m.pasteEvents} pastes · {m.keystrokes} keystrokes · {Math.floor(m.typingDuration/60)}min {m.typingDuration%60}s</div>
            </div>
            <div style={{background:C.bg,borderRadius:10,padding:"10px 13px"}}>
              ...
              <div style={{fontWeight:600,fontSize:16}}>{Math.floor(m.typingDuration/60)}m {m.typingDuration%60}s</div>
            </div>
          </Card>
            ...
            <div style={{marginBottom:12}}>
              ...
              {strengths.map((s,i)=>(
                <div key={i} style={{display:"flex",gap:8,marginBottom:6,padding:"7px 10px",background:C.indigoLight,borderRadius:8,alignItems:"flex-start",border:`1px solid ${C.indigo}20`}}>
                  <span style={{color:C.indigo,fontWeight:600,flexShrink:0}}>✓</span>
        ...
        <div className="sl-grid-2" style={{marginBottom:14}}>
          ...
          <Card style={{background:`linear-gradient(135deg, ${C.dark} 0%, #1B1E2C 100%)`,border:"1px solid rgba(255,255,255,.08)"}}>
            <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(58,47,201,.16)",border:"1px solid rgba(58,47,201,.35)",borderRadius:99,padding:"4px 12px",marginBottom:10}}>
              <span style={{width:5,height:5,background:"#7A70E0",borderRadius:"50%",display:"inline-block"}}/>
              <span style={{fontSize:10,fontWeight:600,color:"#C9C2F5",letterSpacing:.3}}>Job Matches</span>
            </div>
            <h3 style={{fontWeight:600,fontSize:14,color:"#fff",margin:"0 0 12px"}}>Career Pathways</h3>
            {roles.map(ro=>(
              <div key={ro.role} style={{marginBottom:12}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:4,gap:6}}>
                  <span style={{fontSize:12,fontWeight:600,color:"#D2D2CE",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{ro.role}</span>
                  <span style={{fontSize:12,fontWeight:600,color:"#A79CEE",flexShrink:0}}>{ro.prob}%</span>
                </div>
            ...
            <div style={{marginTop:12,background:"rgba(255,255,255,.06)",borderRadius:10,padding:"10px 13px"}}>
              <div style={{fontSize:9,color:"#6B6F7B",marginBottom:2,textTransform:"uppercase"}}>Top Match</div>
              <div style={{fontWeight:600,fontSize:14,color:"#A79CEE"}}>{roles[0].role}</div>
              <div style={{fontSize:11,color:"#6B6F7B"}}>Win Probability: {roles[0].prob}%</div>
            </div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <Card>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                {[
                  {label:"Tab Switches",value:m.tabSwitches||0,flag:(m.tabSwitches||0)>0},
                  {label:"Paste Events",value:m.pasteEvents,flag:m.pasteEvents>0},
                  {label:"Largest Paste",value:`${m.largestPaste}L`,flag:m.largestPaste>10},
                  {label:"Paste Chars",value:m.pasteChars,flag:m.pasteChars>150},
                ].map(m=>(
                  <div key={m.label} style={{background:m.flag?"#FBEAE8":C.bg,border:`1.5px solid ${m.flag?"#F0BEB9":C.border}`,borderRadius:10,padding:"8px 10px"}}>
                    <div style={{fontSize:9,color:C.muted,marginBottom:2,fontWeight:600}}>{m.label}</div>
                    <div style={{fontWeight:700,fontSize:16,color:m.flag?C.red:C.text}}>{m.value}</div>
            ...
            <Card style={{background:C.indigoLight}}>
              <h3 style={{fontWeight:600,fontSize:14,margin:"0 0 10px",color:C.indigo}}>🎯 Skill Gaps to Close</h3>
              {gaps.map((g,i)=>(
        ...
        {/* CTA */}
        <div style={{background:`linear-gradient(135deg, ${C.dark} 0%, #1B1E2C 100%)`,borderRadius:16,padding:isMobile?"20px 16px":"24px 22px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12,border:"1px solid rgba(255,255,255,.08)"}}>
          <div>
            <h3 style={{fontWeight:600,fontSize:15,color:"#fff",margin:"0 0 3px"}}>Ready for your certificate?</h3>
            <p style={{color:"#6B6F7B",fontSize:13,margin:0}}>Earn a shareable proof-of-work from this session.</p>
          </div>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            ...
            <button onClick={()=>setPage("jobs")} className="sl-btn-hover"
              style={{padding:"10px 20px",background:"rgba(255,255,255,.08)",color:"#E4E4E1",border:"1px solid rgba(255,255,255,.15)",borderRadius:10,fontWeight:700,fontSize:13,cursor:"pointer"}}>💼 Browse Jobs →</button>
          </div>
        </div>
      </div>
    </div>
