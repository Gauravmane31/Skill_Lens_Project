...
import { CHALLENGES } from "../constants/constants.js";
import { aiAnalysis, skillGaps, skillGapsFallback } from "../utils/scoring.js";
...
const buildAnalysis = (submissions = []) => {
  ...
  return {
    ...
    confidenceScore,
    recommendedRoles,
    weaknesses: weaknesses.length ? weaknesses : skillGapsFallback(avgCode),
    strengths,
    growthPath:
      confidenceScore >= 80
...
const backendRequest = async (path, options = {}) => {
  ...
  try {
    ...
    console.error(`Request to ${BACKEND_URL}${path} failed:`, networkError);
    throw new Error(
      `Couldn't reach the server${BACKEND_URL ? ` at ${BACKEND_URL}` : ""}. ` +
      "Check that the backend is running and VITE_BACKEND_URL is set correctly.",
    );
  }
...
const authRequest = async (path, body) => {
  ...
  try {
    ...
    console.error(`Auth request to ${BACKEND_URL}${path} failed:`, networkError);
    throw new Error(
      `Couldn't reach the server${BACKEND_URL ? ` at ${BACKEND_URL}` : ""}. ` +
      "Check that the backend is running and VITE_BACKEND_URL is set correctly.",
    );
...
export const confirmPasswordReset = async (newPassword) => {
  const { data, error } = await supabase.auth.updateUser({ password: newPassword });
  if (error) throw error;
  return data;
};

export const signupWithEmail = async ({ name, email, password, accountType, company }) => {
...
const ensureProfile = async (user, overrides = {}) => {
  ...
  if (error) {
    // If the `company` column migration hasn't been applied to this database
    // yet, don't let that break login/submissions/every profile read — retry
    // once without it rather than hard-failing everything that calls this.
    const missingCompanyColumn = /column .*company.* does not exist/i.test(error.message || "");
    if (missingCompanyColumn) {
      console.warn("`profiles.company` column not found — run the latest database/schema.sql migration. Continuing without it for now.");
      const { company: _drop, ...retryPayload } = upsertPayload;
...
const fetchUserSubmissions = async (userId, limit = 500) => {
  const { data, error } = await supabase
    .from("submissions")
    .select("*")
    .eq("user_id", userId)
...
// Loads a user's full submission history from the database and maps each row
// into the same shape SessionPage builds live after a submission, so pages
// like Results/Certificates/Progress work identically whether a result came
// from this session or a past one. Oldest-first, matching the live append order.
export const loadUserResultHistory = async (userId, limit = 500) => {
  const rows = await fetchUserSubmissions(userId, limit);
  return rows
    .slice()
    .reverse()
    .map((row) => ({
      challenge:
        CHALLENGE_BY_ID.get(Number(row.challenge_id)) ||
        { id: row.challenge_id, title: row.challenge_title, xp: 100 },
      code: row.code || "",
      lang: row.lang || "javascript",
      metrics: row.metrics || {},
      integrityScore: Number(row.integrity_score || 0),
      codeScore: Number(row.code_score || 0),
      backendResult: {
        strengths: [],
        weaknesses: [],
        improvementTips: [],
...
export const submitCode = async (payload) => {
  ...
  if (!Number.isInteger(challengeId)) {
    throw new Error("This challenge is missing a valid id. Please return to Challenges and try again.");
  }
  ...
  if (updateError) console.error("Submission saved, but profile stats could not be updated:", updateError);
...
export const fetchUserProfile = async () => {
  const user = await ensureAuthenticatedUser();
  const profile = await ensureProfile(user);
  const submissions = await fetchUserSubmissions(user.id, 500);
